    ...
    <div className="space-y-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center justify-center gap-2 rounded-sm border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-60"
        >
          <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> 새로고침
        </button>
      </header>

      <form onSubmit={search} className="grid gap-3 rounded-sm border border-gray-200 bg-white p-4 sm:grid-cols-[minmax(0,1fr)_180px_auto]">
        <label className="relative">
          ...
          <input
            ...
            className="w-full rounded-sm border border-gray-300 py-2.5 pl-10 pr-3 text-sm outline-none focus:border-[var(--gov-brand)] focus:ring-2 focus:ring-[#dce5f1]"
          />
        </label>
        <select
          aria-label="진행 상태"
          ...
          className="rounded-sm border border-gray-300 px-3 py-2.5 text-sm outline-none focus:border-[var(--gov-brand)]"
        >
